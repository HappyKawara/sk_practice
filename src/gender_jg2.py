...
class gender_judge():
    def __init__(self):
        ...
        for mf in male + female:
            self.bi_name.append(mf[[i:i+2] for i in range(len(mf) - 1)] + ["E" + mf[-2] + mf[-1]] + ["E" + mf[-1]])
        self.gender_list = [1 for i in range(len(male))] + [0 for i in range(len(female))]
        for n in name:
